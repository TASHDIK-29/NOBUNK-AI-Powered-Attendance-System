...
def upgrade() -> None:
    """Upgrade schema."""
    # The student_embeddings table below uses the pgvector VECTOR type, which
    # only exists once the extension is enabled. On a fresh managed database
    # (e.g. Render Postgres) the extension is available but not yet created, so
    # enable it before any table that references the type. Idempotent.
